...
{
    private FrostDictionary<TimerTask, Coroutine> timersTasks = new FrostDictionary<TimerTask, Coroutine>();
...
public class TimerTask
{
    ...
    public virtual void Start()
    {
        if (!Manager.IsStarted(this))
        {
            ...
            Frost.Debug.LogError($"[TimerTask] This timer is already running ! ({timer}/{time})");
    }

    public virtual void Resume()
    {
        if (!Manager.IsStarted(this))
            Frost.Debug.LogError($"[TimerTask] This timer isn't running !");
        else
            isPausing = false;
    }
